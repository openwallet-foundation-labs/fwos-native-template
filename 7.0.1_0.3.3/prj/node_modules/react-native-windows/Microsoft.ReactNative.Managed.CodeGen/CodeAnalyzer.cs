  ...
  public class CodeAnalyzer
  {
    ...
    public ReactAssembly AnalyzeAndConstructAssemblyModel()
    {
      ...
      foreach (var module in assembly.Modules)
      {
        ...
        {
          AddSerializableType(getConstants.Method.ReturnType);
        }
      ...
      if (TryFindAttribute(type, ReactTypes.ReactModuleAttribute, out var attr))
      {
        ...
        foreach (var member in type.GetMembers())
        {
          if (member is IMethodSymbol method)
          {
            ...
            else if (TryExtractGetConstants(method, out var getConstants))
            {
              module.GetConstants.Add(getConstants);
            }
          }
      ...
      [NotNullWhen(returnValue: true)] out ReactMethod? reactMethod)
    {
      ...
      if (TryFindAttribute(method, attributeType, out var attr))
      {
        ...
        for (int i = 0; i < parameters.Length; ++i)
        {
          ...
          if (!synchronous) // IsSynchronous methods don't have support for the callbacks
          {
            if (i == parameters.Length - 1)
            {
              ...
              // if the last argument is a callback, assume this is the resolve function
              if (IsCallback(parameters[i].Type, out resolveParameterCount))
              {
                returnStyle = ReactMethod.MethodReturnStyle.Callback;
                break;
              }
            }
            else if (i == parameters.Length - 2)
            {
              // if the last 2 argument are callbacks, assume these are the resolve and reject functions
              if (IsCallback(parameters[i].Type, out resolveParameterCount) &&
                  IsCallback(parameters[i + 1].Type, out rejectParameterCount))
              {
        ...
        reactMethod = new ReactMethod(method, name, returnStyle.Value, effectiveReturnType,
          effectiveParameters, synchronous, resolveParameterCount, rejectParameterCount);
        return true;
      }
    ...
    private bool TryExtractGetConstants(IMethodSymbol method, [NotNullWhen(returnValue: true)] out ReactGetConstants? getConstants)
    {
      getConstants = null;
      if (TryFindAttribute(method, ReactTypes.ReactGetConstantsAttribute, out _))
      {
        if (!IsAccessible(method))
        {
          m_diagnostics.Add(
            Diagnostic.Create(DiagnosticDescriptors.GetConstantsMustBePublicOrInternal,
              method.GetLocation(),
              method.ReturnType.ToDisplayString(SymbolDisplayFormat.CSharpErrorMessageFormat),
              method.Name,
              method.ContainingSymbol.ToDisplayString(SymbolDisplayFormat.CSharpErrorMessageFormat),
        ...
        {
          m_diagnostics.Add(
            Diagnostic.Create(DiagnosticDescriptors.GetConstantsMustNotReturnVoid,
              method.GetLocation(),
              method.ReturnType.ToDisplayString(SymbolDisplayFormat.CSharpErrorMessageFormat),
              method.Name,
              method.ContainingSymbol.ToDisplayString(SymbolDisplayFormat.CSharpErrorMessageFormat)
        ...
        if (method.ReturnType.SpecialType != SpecialType.None
          || (method.ReturnType.TypeKind != TypeKind.Struct && method.ReturnType.TypeKind != TypeKind.Class))
        {
          m_diagnostics.Add(
            Diagnostic.Create(DiagnosticDescriptors.GetConstantsMustReturnStructOrClass,
              method.GetLocation(),
              method.ReturnType.ToDisplayString(SymbolDisplayFormat.CSharpErrorMessageFormat),
              method.Name,
              method.ContainingSymbol.ToDisplayString(SymbolDisplayFormat.CSharpErrorMessageFormat)
        ...
        {
          m_diagnostics.Add(
            Diagnostic.Create(DiagnosticDescriptors.GetConstantsMustNotHaveParameters,
              method.GetLocation(),
              method.Parameters.Length.ToString(CultureInfo.InvariantCulture),
              method.Name,
              method.ContainingSymbol.ToDisplayString(SymbolDisplayFormat.CSharpErrorMessageFormat)
        ...
        getConstants = new ReactGetConstants(method);
      }
    ...
    private bool IsPromiseType(ITypeSymbol type, [NotNullWhen(returnValue: true)] out ITypeSymbol? typeParameter)
    {
      ...
      if (type != null && type is INamedTypeSymbol namedType)
      {
        if (namedType.IsGenericType)
        {
          if (namedType.ConstructUnboundGenericType().Equals(
        ...
        else if (SymbolEqualityComparer.Default.Equals(namedType, m_reactTypes.ReactPromiseOfVoid))
        {
          typeParameter = m_reactTypes.SystemVoid;
          return true;
        }
      }
    ...
    private static bool IsCallback(ITypeSymbol type, out int parameterCount)
    {
      if (type is INamedTypeSymbol namedType &&
          namedType.DelegateInvokeMethod != null)
      {
        parameterCount = namedType.DelegateInvokeMethod.Parameters.Length;
        return true;
      }
